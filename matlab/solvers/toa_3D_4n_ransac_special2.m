function [x1,y1,inliers1]=toa_3D_4n_ransac_special2(d,sys);
...
bestinliers = 0;
%keyboard;
for kk = 1:sys.ransac_k1
    pp = randperm(n);
    pp = pp(1:6);
    try % skip iteration for nan values

        [sols] = feval(sys.solver,d(:,pp)');
            ...
            for ii = 1:nrsols,
                ...
                for jj = 1:n,
                    ...
                    [y20,inliers0,nr_inliers0,err_rms]= ...
                        toa_trilateration_one_ransac(d(:,jj),xtmp,5,sys.ransac_threshold1);
                    if nr_inliers0==4,
