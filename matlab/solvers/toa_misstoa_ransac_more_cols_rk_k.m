function [sol]=toa_misstoa_ransac_more_cols_rk_k(d,sol,sys);
...
for ii = trycols,
    ...
    for kk =1:sys.ransac_k2;
        ...
        if length(tmp)>=(rk+1),
            tryrows1 = okrows(tmp(1:(rk+1)));
            %trycols2 = sol.cols(trycols1);
            % Equations are d2n = Dhat(1,:)*inv(dr') + x*[1 1;0 V] = zz+x*ZZ;
            zz = inv(dl)*sol.Bhat(:,1);
            ZZ = [ones(length(sol.rows),1) [zeros(1,rk);u]];
            ZZ0 = [[1;zeros(length(sol.rows)-1,1)] [zeros(1,rk);u]];
