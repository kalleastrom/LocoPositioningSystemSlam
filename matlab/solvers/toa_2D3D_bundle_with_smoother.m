function [xopt,yopt,res,jac]=toa_2D3D_bundle_with_smoother(d,x,y,inliers,opts);
...
[xopt,yopt,res,jac]=bundletoa(D,I,J,x,y,1,opts);
...
jac = sparse([II;II;II;II;II],[JJ1;JJ2;JJ3;JJ4;JJ5],[VV1;VV2;VV3;VV4;VV5],nn,3*m+2*n);
...
JJ1x = (j1-1)*2+1+3*m; %All the indices of jac cols that has used first coord of j1
JJ1y = (j1-1)*2+2+3*m; %second coord of j1
JJ2x = (j2-1)*2+1+3*m; %...
JJ2y = (j2-1)*2+2+3*m;
JJ3x = (j3-1)*2+1+3*m;
JJ3y = (j3-1)*2+2+3*m;
...
jac = sparse([IIx;IIx;IIx;IIy;IIy;IIy], ...
    ...
    [VV;-2*VV;VV;VV;-2*VV;VV],2*nn,3*m+2*n);
jac=jac*lambda;
