function [sols,stats] = toa_3D_55(d,settings,xt,yt)
%[sols,stats] = toa_3D_55(d,settings,xt,yt)
% Solver for 'minimal' case TOA node calibration
%  5 senders and 5 receivers
...
if ~isfield(settings,'threshold'); settings.threshold = 10; end;
...
id1 = find(mons1v(nrofunknowns2,:)==0);
id2 = find(mons1v(nrofunknowns2,:)>0);
cfm2 = cfm1(:,[id1 id2]);
...
eqs00 = eqs(1:nrofunknowns2-1)';
for kk = 1:nrofunknowns2-1;
    mm = monomials(eqs(kk));
    cc = coeffs(eqs(kk));
    eqs00(kk) = multipol(cc,mm(1:nrofunknowns2-1,:));
end

etest2  = eqs00;

% settings.basis_size = 110;
% settings.dim = 42;
