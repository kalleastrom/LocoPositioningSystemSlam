function [f,dfdx,okpos]=interp1d_with_derivative_careful(f0,x0,a);
...
elseif 1,
    ...
    tmp = exp(- ((T-Ts).^2 )./(2*a^2))./sqrt(2*a^2*pi);
    oksel = find(isfinite(f0));
    e0 = ones(size(f0));
    f = tmp(:,oksel)*f0(oksel);
    e = tmp(:,oksel)*e0(oksel);
    dfdx=(2*(T-Ts)./(2*a^2)).*exp(- ((T-Ts).^2 )./(2*a^2))./sqrt(2*a^2*pi)*f0;
    tmp2 = (2*(T-Ts)./(2*a^2)).*exp(- ((T-Ts).^2 )./(2*a^2))./sqrt(2*a^2*pi);
    dfdx = tmp2(:,oksel)*f0(oksel);
    okpos = e>0.8;
    %keyboard;
    if turn
        f = f';
