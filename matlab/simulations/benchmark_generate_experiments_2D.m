function [data,data_db]=benchmark_generate_experiments_2D(exlist);
% [data,data_db]=benchmark_generate_experiments_2D(exlist);
...
for exp_k = 1:length(exlist);
    ex = exlist(exp_k);
    for k = 1:ex.NN;
        r = diag([10 10])*rand(2,ex.m); % Random planar placement in room 10x10 meters
        s = diag([10 10])*rand(2,ex.n); % Random planar placement in room 10x10 meters       
        ...
        [ropt,sopt,res,jac]=toa_2D_bundle(d,r,s,Iinl);
